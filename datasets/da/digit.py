...
def load_mnist1(data_dir, raw_data_dir):
    ...
    train_data, train_label = torch.load(
        os.path.join(mnist_dataset.processed_folder, 'training.pt')
    )
    test_data, test_label = torch.load(
        os.path.join(mnist_dataset.processed_folder, 'test.pt')
    )
...
def main(data_dir):
    ...
    raw_data_dir = osp.join(data_dir, 'Digit-Three')
    save_data_dir = osp.join(data_dir, 'digit')

    if not osp.exists(data_dir):
        raise FileNotFoundError('"{}" does not exist'.format(data_dir))

    datasets = ['usps', 'mnist1']
    ...
    for name in datasets:
        ...
        train_dir = osp.join(save_data_dir, name, 'train_images')
        mkdir_if_missing(train_dir)
        test_dir = osp.join(save_data_dir, name, 'test_images')
